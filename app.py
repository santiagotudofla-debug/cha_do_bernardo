...
# Pasta de dados — criada automaticamente ao lado do app.py
_BASE_DIR = Path(__file__).parent
_DATA_DIR = _BASE_DIR / "data"
_DATA_DIR.mkdir(exist_ok=True)

# Arquivo único com todas as informações do convidado + mimo
DATA_FILE = str(_DATA_DIR / "cha_bernardo_confirmacoes.csv")

# Colunas do arquivo unificado (ordem de exibição no Excel)
COLUNAS_CSV = [
    # ── Confirmação – Titular ──────────────────────────────
    "Nome Titular", "Sexo Titular",
    "Fralda Titular", "Formato Fralda Titular", "Valor PIX Fralda Titular (R$)",
    # ── Confirmação – Acompanhante ─────────────────────────
    "Leva Acompanhante",
    "Nome Acompanhante", "Sexo Acompanhante",
    "Fralda Acompanhante", "Formato Fralda Acompanhante", "Valor PIX Fralda Acompanhante (R$)",
    # ── Crianças ──────────────────────────────────────────
    "Qtd Crianças",
    # ── Mimo Extra ────────────────────────────────────────
    "Mimo Escolhido", "Categoria Mimo", "Formato Mimo", "Valor PIX Mimo (R$)",
    # ── Totais ────────────────────────────────────────────
    "Total PIX (R$)",
    # ── Mensagem ──────────────────────────────────────────
    "Mensagem",
...
def inicializar_bancos():
    if not os.path.exists(DATA_FILE):
        pd.DataFrame(columns=COLUNAS_CSV).to_csv(DATA_FILE, index=False)

def salvar_tudo_unificado(dados_rsvp: dict, mimo: dict | None, formato_mimo: str, valor_mimo: float):
    """Salva confirmação + mimo em uma única linha do arquivo CSV unificado."""
    import time
    from datetime import datetime

    total_pix = (
        dados_rsvp.get("Valor PIX Fralda Titular (R$)", 0.0)
        + dados_rsvp.get("Valor PIX Fralda Acompanhante (R$)", 0.0)
        + (valor_mimo if formato_mimo == "PIX" else 0.0)
    )

    linha = {col: "" for col in COLUNAS_CSV}  # garante todas as colunas sempre presentes

    # Titular
    linha["Nome Titular"]                    = dados_rsvp.get("Nome Titular", "")
    linha["Sexo Titular"]                    = dados_rsvp.get("Sexo Titular", "")
    linha["Fralda Titular"]                  = dados_rsvp.get("Fralda Titular", "")
    linha["Formato Fralda Titular"]          = dados_rsvp.get("Formato Fralda Titular", "")
    linha["Valor PIX Fralda Titular (R$)"]   = dados_rsvp.get("Valor PIX Fralda Titular (R$)", 0.0)

    # Acompanhante
    linha["Leva Acompanhante"]               = dados_rsvp.get("Leva Acompanhante", "Não")
    linha["Nome Acompanhante"]               = dados_rsvp.get("Nome Acompanhante", "-")
    linha["Sexo Acompanhante"]               = dados_rsvp.get("Sexo Acompanhante", "-")
    linha["Fralda Acompanhante"]             = dados_rsvp.get("Fralda Acompanhante", "-")
    linha["Formato Fralda Acompanhante"]     = dados_rsvp.get("Formato Fralda Acompanhante", "-")
    linha["Valor PIX Fralda Acompanhante (R$)"] = dados_rsvp.get("Valor PIX Fralda Acompanhante (R$)", 0.0)

    # Crianças
    linha["Qtd Crianças"] = dados_rsvp.get("Qtd Crianças", 0)

    # Mimo
    if mimo:
        linha["Mimo Escolhido"]      = mimo.get("nome", "")
        linha["Categoria Mimo"]      = mimo.get("categoria", "")
        linha["Formato Mimo"]        = formato_mimo
        linha["Valor PIX Mimo (R$)"] = valor_mimo if formato_mimo == "PIX" else 0.0
    else:
        linha["Mimo Escolhido"]      = "Sem mimo"
        linha["Categoria Mimo"]      = "-"
        linha["Formato Mimo"]        = "-"
        linha["Valor PIX Mimo (R$)"] = 0.0

    # Totais e controle
    linha["Total PIX (R$)"]     = round(total_pix, 2)
    linha["Mensagem"]           = dados_rsvp.get("Mensagem", "")
    linha["Data Confirmação"]   = datetime.now().strftime("%d/%m/%Y %H:%M")
    ...
    for tentativa in range(5):
        try:
            ...
            if tentativa == 4:
                st.error("Não foi possível salvar: o arquivo CSV está aberto em outro programa (ex: Excel). Feche-o e tente novamente.")
                raise
            time.sleep(0.5)
...
def checar_fisicos_comprados(nome_item):
    df = pd.read_csv(DATA_FILE)
    if df.empty: return 0
    return ((df["Mimo Escolhido"] == nome_item) & (df["Formato Mimo"] == "Física")).sum()
...
[🟡 Mercado Livre]({info_mimo['link_mercadolivre']})""",
    ...
    if st.button("Confirmar Tudo e Gerar Resumo 🚀"):
        if not nome_titular or sexo_titular == "Selecione...":
            ...
            salvar_tudo_unificado(dados_rsvp, mimo_escolhido, formato_mimo, valor_mimo)
